...
import 'package:moviescout/services/tmdb_title_list_service.dart' show RatingFilter;
...
void main() {
  ...
  group('TmdbTitleRepository', () {
    test('saveTitle and updateTitleMetadata', () async {
      final title = TmdbTitle(tmdbId: 1, name: 'Title 1', mediaType: 'movie', lastUpdated: '2026-07-14', dateRated: DateTime.now());
    ...
    test('saveTitles and updateTitlesMetadata', () async {
      final t1 = TmdbTitle(tmdbId: 1, name: 'T1', mediaType: 'movie', lastUpdated: '2026-07-14', dateRated: DateTime.now());
      final t2 = TmdbTitle(tmdbId: 2, name: 'T2', mediaType: 'tv', lastUpdated: '2026-07-14', dateRated: DateTime.now());
    ...
    test('updateNotifyNewSeasons updates individual and list', () async {
      final t1 = TmdbTitle(tmdbId: 1, name: 'T1', mediaType: 'tv', lastUpdated: '2026-07-14', dateRated: DateTime.now());
      await repository.saveTitle(t1, 'watchlist', 0);
    ...
    test('hasTitlesInList, countTitlesSync, getAllTmdbIds, getAllTitlesInList', () async {
      final t1 = TmdbTitle(tmdbId: 10, name: 'T1', mediaType: 'movie', lastUpdated: '2026-07-14', dateRated: DateTime.now());
      await repository.saveTitle(t1, 'watchlist', 0);
    ...
    test('getUninitializedTitles finds uninitialized', () async {
      final t1 = TmdbTitle(tmdbId: 1, name: 'T1', mediaType: 'movie', lastUpdated: AppConstants.defaultDate, dateRated: DateTime.now());
      await repository.saveTitle(t1, 'watchlist', 0);
    ...
    test('getTitleByTmdbIdSync and getTitlesByTmdbIds', () async {
      ...
      final syncTitle = repository.getTitleByTmdbIdSync('watchlist', 1, 'movie');
    ...
    group('Filtering and Sorting getTitles', () {
      setUp(() async {
        final t1 = TmdbTitle(tmdbId: 1, name: 'Apple', originalName: 'Apple Org', mediaType: 'movie', lastUpdated: '2026-07-14', dateRated: DateTime(2026, 1, 1));
        ...
        final t2 = TmdbTitle(tmdbId: 2, name: 'Banana', originalName: 'Banana Org', mediaType: 'tv', lastUpdated: '2026-07-14', dateRated: DateTime(2025, 1, 1));
      ...
      test('Filter by text', () async {
        final res = await repository.getTitles(listName: 'watchlist', filterText: 'apple');
        expect(res.length, 1);
        expect(res.first.name, 'Apple');
        
        final res2 = await repository.getTitles(listName: 'watchlist', filterText: 'Yellow');
      ...
      test('Filter by genres', () async {
        ...
        final res2 = await repository.getTitles(listName: 'watchlist', filterGenres: [28], filterExcludeGenres: true);
        expect(res2.length, 1);
        expect(res2.first.name, 'Banana'); // Excludes Apple
      });

      test('Filter by providers', () async {
        final res = await repository.getTitles(listName: 'watchlist', filterByProviders: true, filterProvidersIds: [8]);
        expect(res.length, 1);
        expect(res.first.name, 'Banana');
        
        final resEmpty = await repository.getTitles(listName: 'watchlist', filterByProviders: true, filterProvidersIds: []);
        expect(resEmpty.length, 0);
      });

      test('Filter by RatingFilter', () async {
        final resRated = await repository.getTitles(listName: 'watchlist', filterRating: RatingFilter.rated);
        expect(resRated.length, 1);
        expect(resRated.first.name, 'Apple');
        
        final resSeen = await repository.getTitles(listName: 'watchlist', filterRating: RatingFilter.seenOnly);
        expect(resSeen.length, 1);
        expect(resSeen.first.name, 'Banana');
        
        final resFollowing = await repository.getTitles(listName: 'watchlist', filterRating: RatingFilter.followingOnly);
        expect(resFollowing.length, 1);
        expect(resFollowing.first.name, 'Banana');
      });

      test('Filter by pinned', () async {
        final res = await repository.getTitles(listName: 'watchlist', pinned: true);
        expect(res.length, 1);
        expect(res.first.name, 'Apple');
      });
      ...
      test('Sorting variations', () async {
        // userRating
        var res = await repository.getTitles(listName: 'watchlist', sortOption: 'userRating', sortAscending: false);
        expect(res.first.name, 'Apple'); // 8.0 > seenRating
        
        // releaseDate
        res = await repository.getTitles(listName: 'watchlist', sortOption: 'releaseDate', sortAscending: false);
        expect(res.first.name, 'Banana'); // 2026 > 2025
        
        // runtime
        res = await repository.getTitles(listName: 'watchlist', sortOption: 'runtime', sortAscending: false);
        expect(res.first.name, 'Apple'); // 120 > 60
        
        // addedOrder
        res = await repository.getTitles(listName: 'watchlist', sortOption: 'addedOrder', sortAscending: false);
        expect(res.first.name, 'Banana'); // order 1 > order 0
        
        // dateRated
        res = await repository.getTitles(listName: 'watchlist', sortOption: 'dateRated', sortAscending: false);
        expect(res.first.name, 'Apple'); // 2026 > 2025

        // rating (voteAverage)
        res = await repository.getTitles(listName: 'watchlist', sortOption: 'rating', sortAscending: false);
        expect(res.first.name, 'Banana'); // 9.0 > 7.0
      });
      
      test('Pagination in getTitles', () async {
        final res = await repository.getTitles(listName: 'watchlist', offset: 1, limit: 1);
        expect(res.length, 1);
      ...
      test('hasTitlesFiltered and countTitlesFiltered', () async {
        final count = await repository.countTitlesFiltered(listName: 'watchlist', filterText: 'Apple');
        expect(count, 1);
        
        final has = await repository.hasTitlesFiltered(listName: 'watchlist', filterText: 'Apple');
        expect(has, isTrue);
        
        final hasNot = await repository.hasTitlesFiltered(listName: 'watchlist', filterText: 'Strawberry');
