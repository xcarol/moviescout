...
class TestTmdbRateslistService extends TmdbRateslistService {
  ...
  @override
  Future<dynamic> delete(String endpoint, {Map<String, dynamic>? body, ApiVersion version = ApiVersion.v3, String accessToken = ''}) async {
...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockRepository.hasTitlesFiltered(listName: AppConstants.rateslist))
  ...
  group('TmdbRateslistService', () {
    test('updateTitleRate rating > 0 updates server and removes from watchlist if present', () async {
      ...
      when(() => mockRepository.getTitleByTmdbId(AppConstants.watchlist, title.tmdbId, title.mediaType))
          .thenAnswer((_) async => title); // Exists in watchlist
      when(() => mockRepository.deleteTitle(AppConstants.watchlist, title.tmdbId, title.mediaType))
          .thenAnswer((_) async {});
      when(() => mockRepository.saveTitle(title, AppConstants.rateslist, any())).thenAnswer((_) async {});
      when(() => mockRepository.getMaxAddedOrder(AppConstants.rateslist)).thenAnswer((_) async => 0);
      ...
      when(() => mockRepository.updateNotifyNewSeasons(title)).thenAnswer((_) async {});
      ...
      verify(() => mockRepository.saveTitle(title, AppConstants.rateslist, any())).called(1);
      verify(() => mockRepository.updateRating(title)).called(1);
    });

    test('updateTitleRate rating == 0 deletes from server and cleans following config', () async {
      final title = TmdbTitle(tmdbId: 1, mediaType: ApiConstants.tv, name: 'Test', lastUpdated: DateTime.now().toIso8601String(), dateRated: DateTime.now());
      title.notifyNewSeasons = true;
      title.rating = 8.0;
      title.inLists = [AppConstants.rateslist];

      when(() => mockFollowingService.removeFollowingFromServer(title)).thenAnswer((_) async => true);
      when(() => mockRepository.deleteTitle(AppConstants.rateslist, title.tmdbId, title.mediaType)).thenAnswer((_) async {});
      when(() => mockRepository.getTitleGlobal(title.tmdbId, title.mediaType)).thenAnswer((_) async => null);
      ...
      verify(() => mockFollowingService.removeFollowingFromServer(title)).called(1);
      verify(() => mockRepository.deleteTitle(AppConstants.rateslist, title.tmdbId, title.mediaType)).called(1);
    });
    ...
    test('toggleNotify adds following if not following', () async {
      ...
      when(() => mockRepository.updateNotifyNewSeasons(title)).thenAnswer((_) async {});
      when(() => mockFollowingService.addFollowingToServer(title)).thenAnswer((_) async => true);
