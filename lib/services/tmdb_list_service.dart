...
class TmdbListService extends TmdbBaseService with ChangeNotifier {
  ...
  Future<void> retrieveList(
    ...
    try {
      ...
      if (_titles.isNotEmpty) {
        _titles =
            await _syncWithServer(accountId, _titles, retrieveMovies, retrieveTvshows);
      } else {
  ...
  Future<List<TmdbTitle>> _retrieveLocalList() async {
    try {
      ...
      final r = await isar.tmdbTitles.where().findAll();
      return r.toList();
    } catch (e) {
      SnackMessage.showSnackBar(
  ...
  Future<void> _updateLocalList() async {
    ...
    // Obre una transacció per escriure a Isar
    await isar.writeTxn(() async {
      // Esborra tots els títols anteriors (opcional segons la lògica)
      await isar.tmdbTitles.clear(); // Si vols substituir-los completament

      // Afegeix o actualitza tots els títols
