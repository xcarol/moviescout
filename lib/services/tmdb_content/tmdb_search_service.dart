...
class TmdbSearchService extends TmdbBaseListService<TmdbItem> {
  ...
  Future<List<TmdbItem>> _getAllSortedAndFiltered() async {
    ...
    if (filterMediaType == '' ||
      ...
      final titles = await titleRepository.getTitles(
        ...
        filterText: '',
        limit: 1000,
        sortOption: selectedSort == SortOption.relevance ? SortOption.addedOrder : SortOption.alphabetically,
      );
      if (filterMediaType == '') {
  ...
  Future<void> retrieveNluSearchlist(String searchTerm) async {
    ...
    try {
      ...
      // Executar la inferència ONNX en l'isolate i cercar a la base de dades
      final tmdbIds = await NluInferenceService.search(searchTerm);

      if (tmdbIds.isNotEmpty) {
        final existingTitles = await titleRepository.getTitlesByTmdbIds(tmdbIds);
        ...
        for (var id in tmdbIds) {
          if (existingMap.containsKey(id)) {
            ...
            // Creem un TmdbTitle bàsic per forçar-ne l'actualització des de l'API
        ...
        // Obtenim els detalls des de l'API per a cadascun
        ...
        // Guardem a la llista corresponent
