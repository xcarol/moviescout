...
This section documents the steps required to configure the backend and authentication environment for MovieScout (Supabase migration and Google Sign-In). Follow these steps if you ever need to rebuild the environment from scratch.

### 1. Google Cloud & Firebase Setup

Google Sign-In in Flutter apps requires specific configuration in the Google Cloud Platform (GCP). If your project uses Firebase (e.g., for Crashlytics), it is highly recommended to link them.

#### A. Debug SHA-1 Fingerprint (Crucial for Android)
Without this, the app will throw a `[16] Account reauth failed` or `provider_disabled` error when returning from the Google popup.

1. Extract the SHA-1 fingerprint from your development machine:
   ```bash
   keytool -list -v -keystore ~/.android/debug.keystore -alias androiddebugkey -storepass android -keypass android
   ```
2. Go to the [Firebase Console](https://console.firebase.google.com).
3. Navigate to **Project Settings** (gear icon) > **General** tab.
4. Under the "Your apps" section, select the MovieScout Android app (`com.xicra.moviescout`).
5. Click **Add fingerprint** and paste the SHA-1 generated in step 1.

#### B. Obtain Web Credentials (Client ID and Secret)
Despite being an Android/iOS app, Supabase uses the Google **Web Client** token to validate the session.

1. Go to [Google Cloud Console > Credentials](https://console.cloud.google.com/apis/credentials).
2. Ensure you have selected the correct project.
3. In the **OAuth 2.0 Client IDs** list, find the one of type **Web application**.
4. Click on it and copy two values:
   * **Client ID**
   * **Client Secret**
5. Save the Client ID in the `.env` file of the app using the key `GOOGLE_WEB_CLIENT_ID`.

### 2. Supabase Configuration

Supabase needs to allow Google logins and accept the token generated by our app.

1. Go to your project dashboard on [Supabase](https://supabase.com/dashboard).
2. Navigate to **Authentication** > **Providers**.
3. Find **Google** and toggle **Enable Google** to on.
4. Fill in the fields with the information obtained in Step 1.B:
   * **Client ID**: (Your Web *Client ID*).
   * **Client Secret**: (Your Web *Client Secret*).
5. Save the changes.

#### A. Authorize the Callback URL
To ensure secure communication between Google and Supabase without being blocked:
1. In the same Supabase menu where you just enabled Google, copy the provided **Callback URL** (it ends in `/auth/v1/callback`).
